...
Le dashboard tagliano lo storico per esercizio e rispondono a "come sto andando nelle trazioni". Manca(va) l'altra lettura, quella che il diario su Keep dava gratis: "cosa ho fatto martedi'". E' una schermata a se' (`/diary`), raggiungibile dalla home, che elenca gli allenamenti dal piu' recente: data, tipo (o nome del giorno di scheda) e una riga per esercizio col riepilogo. Toccando una giornata si apre il dettaglio: serie per serie, note della voce e dell'allenamento, link ai progressi di quell'esercizio. Da li' si corregge anche cio' che si e' registrato male, ed e' la ragione per cui esisteva un buco: prima l'unico modo di sistemare un numero sbagliato era la dashboard di Supabase. Nel dettaglio aperto di una giornata si puo':
- correggere una voce, con gli stessi widget del logging (`ExerciseCard` riusata): spuntare o togliere una serie, cambiare scheme, zavorra, condizione e note. Il totale della metrica non si scrive a mano: lo ricalcola il registro (§4) dalle serie spuntate, o resterebbe un numero che non corrisponde piu' al dettaglio;
- aggiungere un esercizio dimenticato, che si aggancia a QUELLA giornata invece di creare una seconda sessione nella stessa data. Va in fondo all'ordine di esecuzione e mai dentro un superset: agganciare e' una decisione presa mentre ci si allena, non mentre si corregge;
- cancellare una voce (stesso gesto e stessa conferma dell'elenco in §6), oppure l'intera giornata — scorciatoia per il doppio salvataggio o la sessione di prova, con una conferma che dice quante voci porta via.
Si salta anche a una data col calendario. I comandi stanno solo dentro il dettaglio aperto: il diario si usa per leggere, e una matita su ogni riga trasformerebbe un elenco da scorrere in un modulo da toccare per sbaglio.
